...
```{r}
model.data <-  (confirmed %>% mutate(timestamp=as.numeric(as.POSIXct(date))) %>% 
...
```{r}
lag.data <- function(data, mobility, .lag) {
  data %>%
    inner_join(mobility) %>%
...
```{r}
seq(0,30,1) %>% 
  map(~lag.data(confirmed %>% inner_join(mobility.chg), mobility.data %>% inner_join(mobility.chg), .)) %>% 
  bind_rows() %>% 
  group_by(country_region, lag) %>% 
  summarise(cor=cor(diff, overall_percent_change_from_baseline)) %>% 
  ungroup() %>% 
...
mobility.data %>% 
  ...
  ggplot(aes(x=date, y=overall_percent_change_from_baseline))+
  geom_line()+
  facet_wrap(~country_region,  scales="free") +
  scale_x_date(date_breaks = "2 day") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
