...
mobility.data.model.data <- (
  mobility.data %>%
    ...
    fit.loess.model("overall_percent_change_from_baseline", "timestamp")
)$data
```
...
mobility.data.model.data %>%
  ...
  plot.model.data(.feature1 = "date", .feature2 = "overall_percent_change_from_baseline")
```
...
corr.data.model %>%
  plot.model.data(.feature1 = "lag", .feature2 = "cor", is.date=F)
```
...
mobility.data.model.data <-
  (
    mobility.data %>% filter(
      !is.na(overall_percent_change_from_baseline_diff) &
        !is.infinite(overall_percent_change_from_baseline_diff)
    ) %>%
      group_by(country_region, sub_region_1) %>% fit.loess.model("overall_percent_change_from_baseline_diff", "timestamp")
  )$data
```
...
mobility.data.model.data %>%
  ...
  plot.model.data(.feature1 = "date", .feature2 = "overall_percent_change_from_baseline_diff")
```
